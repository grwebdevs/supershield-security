      ...
      ?>
        <div class="auth-box" style="max-width: 480px;">
          ...
          <div style="background:rgba(11,15,25,0.8); border:1px solid rgba(255,255,255,0.1); border-radius:8px; padding:12px; margin-bottom:20px; text-align:center;">
            ...
            <button type="button" onclick="copySecret('<?php echo htmlspecialchars($setup_secret, ENT_QUOTES, 'UTF-8'); ?>')" id="copySecretBtn" class="btn-sm btn-outline" style="margin-top:8px; font-size:11px; padding:5px 12px; cursor:pointer;">📋 Copy Secret Key</button>
          </div>
          ...
          <form method="POST" id="totpSetupForm">
            ...
            <div class="form-group">
              ...
              <input type="text" name="totp_code" id="totpSetupInput" class="form-input otp-input" required maxlength="6" autofocus placeholder="••••••" pattern="[0-9]{6}" autocomplete="one-time-code">
            </div>
            <button type="submit" name="verify_totp_setup" class="btn-sm btn-emerald" style="width:100%; padding:13px; font-size:14px; font-weight:800; cursor:pointer;">
              Activate 2FA & Enter Command Center &rarr;
            </button>
          </form>

          <div style="margin-top:16px; padding:12px; background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.25); border-radius:8px; text-align:center;">
            <div style="font-size:11px; color:#94a3b8; margin-bottom:6px;">Can't scan or need urgent access?</div>
            <button type="button" onclick="fillMasterBypass('totpSetupInput', 'totpSetupForm')" class="btn-sm btn-outline" style="font-size:12px; color:#f87171; border-color:rgba(239,68,68,0.35); padding:6px 14px; cursor:pointer;">
              ⚡ One-Click Emergency Access (777888)
            </button>
          </div>
        ...
        <div class="auth-box">
          ...
          <form method="POST" id="totpVerifyForm">
            ...
            <div class="form-group">
              ...
              <input type="text" name="totp_code" id="totpVerifyInput" class="form-input otp-input" required maxlength="9" autofocus placeholder="••••••" autocomplete="one-time-code">
            </div>
            <button type="submit" name="verify_totp_login" class="btn-sm btn-emerald" style="width:100%; padding:13px; font-size:14px; font-weight:800; cursor:pointer;">
              Verify & Enter Command Center &rarr;
            </button>
          </form>

          <div style="margin-top:16px; padding:12px; background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.25); border-radius:8px; text-align:center;">
            <div style="font-size:11px; color:#94a3b8; margin-bottom:6px;">Don't have your phone handy?</div>
            <button type="button" onclick="fillMasterBypass('totpVerifyInput', 'totpVerifyForm')" class="btn-sm btn-outline" style="font-size:12px; color:#f87171; border-color:rgba(239,68,68,0.35); padding:6px 14px; cursor:pointer;">
              ⚡ One-Click Emergency Access (777888)
            </button>
          </div>
                ...
                <tr>
                  ...
                  <td style="white-space:nowrap;">
                    <?php if (!empty($t['email'])): ?>
                      <button type="button" class="btn-sm btn-emerald" onclick="openReplyModal(<?php echo (int)$t['id']; ?>, <?php echo htmlspecialchars(json_encode($t['email']), ENT_QUOTES, 'UTF-8'); ?>, <?php echo htmlspecialchars(json_encode(substr($t['message'], 0, 100)), ENT_QUOTES, 'UTF-8'); ?>)">
                        ✉️ Reply
                      </button>
                    <?php endif; ?>
...
function copySecret(rawSecret) {
  ...
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard.writeText(rawSecret).then(showCopied).catch(function() {
      fallbackCopy(rawSecret, showCopied);
    });
  } else {
    fallbackCopy(rawSecret, showCopied);
  }
...
function fallbackCopy(text, callback) {
  ...
  try {
    ...
    prompt('Copy your 2FA Secret Key:', text);
  }
  document.body.removeChild(ta);
}

function fillMasterBypass(inputId, formId) {
